...
- [ ] Answer the remaining **4** P0 business questions in [`OPEN_BUSINESS_QUESTIONS.md`](OPEN_BUSINESS_QUESTIONS.md)
  - Priority: P0
  - Source: EVENT-013, narrowed by EVENT-014
  - Notes: Q-001 (provider), Q-002 (legal), Q-020 (PromptPay refund mechanism), BQ-030 (**stacking only** — Phase F/promotion engine). **The fee numbers are now answered**: DEC-035 sets a flat ฿10 (1000 satang) delivery fee and DEC-036 a fixed ฿5 (500 satang) service fee, both approved 2026-08-24 — so BQ-026 is closed. **The commission rate is now answered too**: DEC-043 sets it at 8% of the food subtotal, rounded to whole baht, approved 2026-09-05 — so Q-010/BQ-028 is closed, and **commission ledger-posting code is implemented** (`postCommissionLedger`). **The service fee is now fully answered**: DEC-047 (2026-09-06) recognizes it as `PLATFORM_REVENUE` at payment success, implemented (`postServiceFeeLedger`); DEC-048 (2026-09-06) resolves refundability — included in an eligible full order refund, not implemented — so **BQ-027 is closed in full** (Q-020's refund mechanism and BQ-016/BQ-015/BQ-017/BQ-031's own refund-cause questions remain separately `OPEN` and are unaffected by DEC-048). **The promotion/discount funder model is now answered too**: DEC-046 (2026-09-05) sets it at per-promotion funder, `PLATFORM` or `MERCHANT` only, no split — so BQ-030 retains only its stacking half (no promotion engine exists to build against it yet). **The cooked-food cost is now answered too**: DEC-051 (2026-09-07) allocates it by cause — platform-caused loss to BANHAO as `PLATFORM_WRITE_OFF`, merchant-caused to the merchant, from `PREPARING`, valued at `orders.subtotal_satang` — so BQ-015 is closed (policy only; runtime not implemented, and BQ-017/BQ-031/Q-020/BQ-024/BQ-013 are unaffected). **Every structural question is answered** by DEC-016…DEC-032; what remains is the provider, legal, and a stacking question. BQ-023, BQ-033 and Q-004 are **deferred with COD** (DEC-016), not answered.
