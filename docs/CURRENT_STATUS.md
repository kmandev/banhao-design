...
| 8 | Driver R-13 earnings, and any money on a rider surface | **Engineering task** (business decision resolved) | **BQ-029 resolved 2026-09-05 — DEC-044**: flat ฿12 (1200 satang) per completed delivery. `deliveries.rider_earning_satang` still stays NULL today — computing and writing it, and posting a `RIDER_PAYABLE` ledger entry, is a separate, not-yet-authorized implementation task, not a further business decision |
