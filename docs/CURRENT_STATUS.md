...
| **F** — Payment on `NullPaymentProvider` | Provider interface, null provider, payment attempts and expiry, webhook simulator, event processing, `reconciliation_cases` | Implemented. **Three ledger groups are now written**: `MERCHANT_COMMISSION` (DEC-043, 8% of the food subtotal, round to whole baht) and `CUSTOMER_PAYMENT` (`+grand_total`, 2026-09-05) at payment success, and `RIDER_EARNING` (`RIDER_PAYABLE −1200` / `PLATFORM_WRITE_OFF +200`, DEC-044/045) at delivery completion; a **read-only** payment ↔ `CUSTOMER_PAYMENT` reconciliation followed 2026-09-06. **"The ledger balances to zero" is still not a testable claim** — gross merchant food payable is unposted, delivery-fee revenue is unrecognized, and the service fee has a locked recognition rule (**DEC-047**) but no posting code. No settlement, no payout, no refund posting exists |
...
| 16 | Service fee revenue recognition — the ฿5 (500 satang) service fee has never been recognized as `PLATFORM_REVENUE` | **Engineering task** (business decision resolved) | **DEC-047 locked 2026-09-06 — implementation pending.** Recognized at the successful-payment economic-finality point, in its own `SERVICE_FEE_REVENUE` group, amount read from the immutable `orders.service_fee_satang`, positive sign, no new account and no migration (`docs/SETTLEMENT_MODEL.md` § 3.2). **No posting code is written**, and the decision closes neither BQ-027 (refundability), nor delivery-fee revenue recognition, nor gross merchant payable, nor order-level zero-sum |
