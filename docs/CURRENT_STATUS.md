...
| 8 | Driver R-13 earnings, and any money on a rider surface | **Engineering task** (business decisions resolved) | **BQ-029 resolved 2026-09-05 — DEC-044** (flat ฿12/1200 satang per completed delivery). **Implemented** `6432e3b3`: `deliveries.rider_earning_satang` is snapshotted at delivery completion and a `RIDER_PAYABLE −1200` ledger entry is posted. That implementation surfaced a real accounting boundary — the entry has no offsetting funding entry, since no `CUSTOMER_PAYMENT` line exists anywhere in the current ledger — which **BQ-040 / DEC-045 (2026-09-05) resolved**: BANHAO absorbs the ฿2 (200 satang) gap between the ฿10 delivery fee and the ฿12 rider earning as a `PLATFORM_WRITE_OFF`. **The offsetting `PLATFORM_WRITE_OFF` entry itself is not yet implemented** — DEC-045 is a documentation lock only, and no rider earnings surface exists in any app yet |
