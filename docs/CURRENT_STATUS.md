...
| 15 | Whether `payment_transactions` and `CUSTOMER_PAYMENT` are ever verified to agree | **Engineering task** (design locked, not yet built) | **Reconciliation design locked 2026-09-05** (`docs/SETTLEMENT_MODEL.md` § 11.1) — payment-transaction-centric matching by the reconstructed `group_key`, restricted to the earliest `payment_transactions` row per `SUCCESS` payment (excluding surplus by construction). No new DEC number. **Read-only, not implemented** — and the smallest version (a query/report) cannot yet persist findings into `reconciliation_cases` without a separate migration, since that table's `kind` CHECK has no room for the new categories |
