...
Three documents: `docs/TECHNICAL_ARCHITECTURE.md` (22 sections), `docs/ARCHITECTURE_DECISIONS.md` (**ADR-001…ADR-012, all `PROPOSED` at the time**), `docs/OPEN_TECHNICAL_QUESTIONS.md` (**TQ-001…TQ-016**). **No business decision changed; no question closed.** *(Status update: all 12 ADRs were ratified `ACCEPTED` on 2026-08-12 via Application Architecture V1.1 §16 — see `docs/ARCHITECTURE_DECISIONS.md`. This does not change what was true when EVENT-015 happened.)*
...
**`feature/supabase-migration-v1` is merged into `main`** (merge commit `e471ec1d`, 2026-08-11), and so is `feature/database-design-v1` beneath it. Both branches have zero unmerged commits relative to `main`; the 16 migration files on the branch and on `main` are byte-identical. The database schema is **LOCKED at checkpoint `e471ec1d`** — see `docs/CURRENT_STATUS.md` and `CLAUDE.md`. Do not treat this branch as pending review, and do not attempt to merge it again.

## Immediate Next Step

**Migration V1 is already merged and the schema is LOCKED — this is not a pending approval gate.** Whether the 16 migrations have additionally been *applied to the live* `banhao-dev` project is tracked separately: `docs/CURRENT_STATUS.md` carries the current provenance/verification record on that point, and it must be read there, not guessed or inferred from this file. Do not assert applied or not-applied yourself, and do not run `supabase db push`, `supabase link`, or any command against the live project without an explicit instruction independent of this document. `DBQ-004` (bank account storage) and `DBQ-011` (order number format) didn't block writing the schema — the design's own column shapes were sufficient — but the actual encryption approach and number-generation algorithm are still `OPEN` and live in application code, not this schema.

One **code** task remains: `RolesGuard`, `set_user_role()` and the `enforce_profile_immutable_columns()` trigger all read `profiles.role`, which DEC-033 deprecated. The schema half is done — `platform_staff`, `restaurant_members`, and `riders` all exist in the merged migrations. The column cannot be dropped until the code resolves capability from those tables instead — tracked in `docs/TODO.md`, and gated on the live-apply verification above before any backfill/drop step can run.

Also still open: **architecture review of `docs/TECHNICAL_ARCHITECTURE.md`.** ADR-001…ADR-012 are **no longer `PROPOSED` — all 12 are `ACCEPTED`**, ratified 2026-08-12 via Application Architecture V1.1 §16 (see `docs/ARCHITECTURE_DECISIONS.md`). Three technical questions are **T0 and block backend work**: **TQ-011** (migration workflow — agree it before the first domain migration, not after), **TQ-012** (how concurrency correctness is *proved* — the EVENT-007 precedent of verifying by execution against real PostgreSQL applies), and **TQ-008** (provider adapter, gated on Q-001/Q-020).
...
- 16 migrations exist on `main` (merged from `feature/supabase-migration-v1` at `e471ec1d`, EVENT-018/019) and the schema is LOCKED — read `docs/DATABASE_MIGRATION_V1_REPORT.md` before writing another one. Never `supabase db push` or `supabase link` to the live project without explicit instruction. Whether they are applied to the live project is tracked in `docs/CURRENT_STATUS.md`'s provenance note — do not assume either way.
...
- Do not implement anything tagged `PROPOSED` in the technical documents. **ADR-001…ADR-012 are `ACCEPTED`** (ratified 2026-08-12 — see `docs/ARCHITECTURE_DECISIONS.md`), so they may be built on; `TQ-NNN` items are separate and must not be turned into assumptions.
